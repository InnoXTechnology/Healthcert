	...
	<div class="containerMain" style="min-height:500px;">
		...
		<div class="side" style="margin-top:30px">
			...
			<table id="bgTable" border="1" cellspacing="1" cellpadding="6" width="700px" style="margin:auto;">
				<tr align="center" style="background:rgba(110, 158, 48, 0.5); color:#3F3F3F;"><td>วันที่ขอ</td><td>สถานะ</td><td>การกระทำ</td></tr>
				<?php foreach ($requests as $request): ?>
				<tr align="center" style="color:#3F3F3F"><td><?= $request['Request']['receipt_no']; ?></td>
					<?php $date = date_create($request['Request']['receipt_date']); ?><td><?= $request['Request']['status']; ?></td>
					...
					location.href='".$this->Html->url(array('action' => 'delete_by_id',$request['Request']['id']))."'" ,'class' => 'btn btn-danger', 'style' => 'width:50px;')); ?></td></tr>
				<?php endforeach; ?>
			</table>
			<br/>
		</div>
